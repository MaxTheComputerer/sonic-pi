...
module SonicPi
  class Style
    ...
    STYLE_LOOKUP = {
      ...
      viennese_waltz: {
        0 => [NormalDistribution.new, NormalDistribution.new(-0.3057, 0.02036), NormalDistribution.new(-0.039055, 0.2109)]
      },
    ...
    def compatible_with?(metre)
      (0..@deepest_metrical_level).each do |level|
        ...
        return false if @distributions[level].length != metre_at_level.length
      end
      return true
    end
